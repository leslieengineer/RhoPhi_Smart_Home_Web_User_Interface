...
```typescript
type NodeModelType = 'onoff' | 'lightness' | 'generic'
...
interface SceneTarget {
  addr: string // Target node address
  relay_on: boolean
  brightness: number
}
...
type FirmwareEvent =
  ...
  | { type: 'SCENE_ACTIVATED'; payload: { id: number } }
  | { type: 'PONG' }
...
| POST   | `/api/device/relay`               | `{ state: boolean }`          |
| POST   | `/api/device/brightness`          | `{ level: number }`           |
| GET    | `/api/mesh/nodes`                 | MeshNetwork                   |
| POST   | `/api/mesh/node/:addr/relay`      | `{ state: boolean }`          |
| POST   | `/api/mesh/node/:addr/brightness` | `{ level: number }`           |
...
| --------- | ------------- | ---------------------------- | --- |
| `state`   | `DeviceState` | REST GET `/api/device/state` |
| `loading` | `boolean`     | —                            |
| `error`   | `string       | null`                        | —   |

**Actions**: `fetchState()`, `setRelay(on)` _(optimistic)_, `setBrightness(level)` _(optimistic)_, `applyServerUpdate(payload)` _(WebSocket push)_.

### `useMeshStore` (`src/stores/mesh.ts`)

| State      | Type                    | Nguồn dữ liệu              |
| ---------- | ----------------------- | -------------------------- |
| `nodes`    | `Map<string, MeshNode>` | REST + WS push             |
| `selfAddr` | `string`                | REST GET `/api/mesh/nodes` |
| `loading`  | `boolean`               | —                          |

**Computed**: `nodesArray`, `onlineCount`, `nodeCount`

**Actions**: `fetchNodes()`, `toggleNode(addr, state)`, `setNodeBrightness()`, `renameNode()`, `removeNode()`, `updateNode()` _(WS)_, `markOffline()` _(WS)_.
...
| State        | Type      | Nguồn dữ liệu          |
| ------------ | --------- | ---------------------- | --------------------- |
| `scenes`     | `Scene[]` | REST GET `/api/scenes` |
| `loading`    | `boolean` | —                      |
| `activating` | `number   | null`                  | ID đang được activate |

**Actions**: `fetchScenes()`, `createScene(name, targets[])`, `activateScene(id)`, `deleteScene(id)`.
...
wsService.onMessage(msg) → switch(msg.type) →
  STATE_UPDATE   → deviceStore.applyServerUpdate()
  NODE_UPDATE    → meshStore.updateNode()
...
[ This Device Card ]            ← Relay toggle + Brightness slider
[ Mesh Quick List (top 3) ]    ← Tên node + status dot + relay toggle
```
...
[ Online X / Total Y ]
[ NodeCard * N ]     ← relay + brightness (nếu lightness model) + RSSI bar
...
- Editable name input
- Grid thông tin: Address, Model, Status, RSSI
...
**Dữ liệu cần**: `scenesStore`, `meshStore` (cho create modal)
...
[ SceneCard * N ]
  ├─ Tên scene + target summary
  ├─ Target chips (addr + relay state)
  └─ [▶ Activate] button
```
...
- Input: Scene Name
- Danh sách online nodes với inline toggle + brightness slider
- Submit → `scenes.createScene(name, targets[])`
...
WebServerManager::init()
  ...
  └── registerHandlers_()
        ...
        ├── GET  /api/device/state  → relay_on, brightness, scene_id, device_name
        ├── POST /api/device/relay  → set relay (TODO: peripheral control)
        ├── POST /api/device/brightness → set PWM level
        ├── GET  /api/mesh/nodes    → MeshNetwork (TODO: MeshNodeDB)
        ├── POST /api/mesh/node/*   → node relay/brightness/name (TODO: BLE Mesh)
        ├── DELETE /api/mesh/node/* → remove node
        ├── GET+POST /api/scenes    → list + create scenes (TODO: SceneList)
...
| API Endpoint                  | TODO                                                |
| ----------------------------- | --------------------------------------------------- |
| `/api/device/relay`           | Wire to peripheral/relay control                    |
| `/api/device/brightness`      | Wire to LEDC PWM                                    |
| `/api/mesh/nodes`             | Wire to MeshNodeDB                                  |
| `/api/mesh/node/*/relay`      | Wire to BLE Mesh Generic OnOff Set                  |
| `/api/mesh/node/*/brightness` | Wire to BLE Mesh Lightness Set                      |
| `/api/mesh/node/*/name`       | Persist to NVS namespace `mesh_nodes`               |
| `/api/scenes`                 | Wire to SceneList, persist to NVS                   |
...
| `STATE_UPDATE` | Khi relay/brightness thay đổi   | On-change  |
...
### Ví dụ: User bật relay trên MeshView

```
User clicks RelayToggle
  │
  ▼
RelayToggle emits update:modelValue=true
  │
  ▼
MeshView: mesh.toggleNode(addr, true)
  │
  ├─► Optimistic: nodes.get(addr).relay_on = true  (UI cập nhật ngay)
  │
  └─► api.setNodeRelay(addr, true)
        │  POST /api/mesh/node/0x0002/relay {"state":true}
        ▼
      ESP32 Firmware
        │  BLE Mesh: Generic OnOff Set → node 0x0002
        │
        ▼
      Node xử lý, gửi status ack
        │
        ▼
      Firmware broadcast WS: {"type":"NODE_UPDATE","payload":{...relay_on:true...}}
        │
        ▼
      wsService.onMessage → useWsStore → meshStore.updateNode()
        │
        ▼
...
# Kỳ vọng: {"relay_on":false,"brightness":50,"scene_id":0,"device_name":"RhoPhi Gateway"}
```

**Bật relay:**

```bash
curl -X POST http://192.168.4.1/api/device/relay \
...
# Kỳ vọng: {"self_addr":"0x0001","node_count":0,"online_count":0,"nodes":[]}
```
